...
<?php
	...
	if (!empty($_POST['usr_name']) and !empty($_POST['usr_email']) and !empty($_POST['usr_id'])){
        if (countSql(mysqli_connect(MYSQL_SERVER, MYSQL_ADMIN, MYSQL_TOCKEN, MYSQL_LOCATION), "SELECT * FROM tblusers WHERE usr_email='$usr_email' and usr_id<>'$usr_id'") == 0){
            $sql = "UPDATE tblusers SET usr_name='$usr_name', usr_email='$usr_email', usr_type='$usr_type' WHERE usr_id='$usr_id'";
